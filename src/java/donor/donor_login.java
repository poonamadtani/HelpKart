 ...
 * @author AI-LAPTOP
 */
@WebServlet(name = "donor_login", urlPatterns = {"/donor_login"})
public class donor_login extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       String email = request.getParameter("email");
        ...
        ResultSet rs = DataManager.getData("Select * from donor_master  where email_id='" + email + "' and pass='" + password + "'");
        try {
            if (rs.next()) {
                ...
                session.setAttribute("phoneno", rs.getString(7));
                session.setAttribute("city", rs.getString(8));
                session.setAttribute("secque", rs.getString(9));
                session.setAttribute("answer", rs.getString(10));
                response.sendRedirect("donor/home.jsp");
            }else{                

                response.sendRedirect("donor/donor_login.jsp?error=" +rs);
            }
